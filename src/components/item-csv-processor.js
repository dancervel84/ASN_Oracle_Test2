...
export class ItemCSVProcessor {
    ...
    async processItems(itemsData) {
        ...
        for (let i = 0; i < itemsData.length; i += batchSize) {
            ...
            try {
                // Convert req_batch_nbr_flg to proper format
                const processedBatch = batch.map(item => ({
                    ...item,
                    req_batch_nbr_flg: item.req_batch_nbr_flg === 'yes' || item.req_batch_nbr_flg === '1' || item.req_batch_nbr_flg === 'true' ? 'yes' : ''
                }));
                
                await this.itemApiService.sendItem(processedBatch);
                results.push({ status: 'fulfilled', value: `Lote ${Math.floor(i/batchSize) + 1}` });
            } catch (error) {
                results.push({ 
                    status: 'rejected', 
                    reason: `Lote ${Math.floor(i/batchSize) + 1}: ${error.message}` 
                });
            }
        }
    ...
    showResults(results) {
        ...
        let summaryMessage = `Proceso completado. ${successes.length} de ${results.length} lotes de artículos enviados con éxito.`;
