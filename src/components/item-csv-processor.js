...
export class ItemCSVProcessor {
    ...
    parseFileData(data) {
        return data.filter(item => {
            ...
            return item.company_code && item.part_a && item.description;
        }).map(item => ({
            company_code: item.company_code,
            part_a: item.part_a,
            description: item.description,
            barcode: item.barcode,
            max_case_qty: item.max_case_qty,
            lpns_per_tier: item.lpns_per_tier,
            tiers_per_pallet: item.tiers_per_pallet,
            req_batch_nbr_flg: item.req_batch_nbr_flg === 'yes' || item.req_batch_nbr_flg === 'true' || item.req_batch_nbr_flg === '1' ? 'yes' : '',
            product_life: item.product_life,
            description_2: item.description_2,
            description_3: item.description_3,
            primary_uom_code: item.primary_uom_code,
            pack_uom_code: item.pack_uom_code
        }));
    }
}