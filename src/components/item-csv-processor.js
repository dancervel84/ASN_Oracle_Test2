...
export class ItemCSVProcessor {
    ...
    async processFile(file, fileExtension) {
        ...
        try {
            ...
            if (success) {
                this.modal.show('success', 'Éxito', `${itemsData.length} artículos han sido procesados y enviados correctamente a WMS.`);
            }
    ...
    parseFileData(data) {
        return data.filter(row => {
            ...
            return row.company_code && row.part_a && row.description;
        }).map(row => ({
